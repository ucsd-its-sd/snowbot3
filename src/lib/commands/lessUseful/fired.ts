...
export class FiredHiredCommand extends Command {
    ...
    readonly hireJokes = [' is still gonna be fired actually.', ' has been rehired! All your shifts start at 6:45am.', ' Welcome back! We have a thunderbird user on the line for you.',
    ...
    execute(msg: Message, match: CommandMatch, state: IStateContainer<State>) {
        if (match.groups.fired) {
            ...
            if (fireable.length == 0) {
                msg.channel.send('https://tenor.com/view/no-i-dont-think-i-will-gif-23864982');
                return;
            }
            ...
            let joke = this.hireJokes[Math.floor(Math.random() * this.hireJokes.length)];
